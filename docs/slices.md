...
| `slider_images` | [SliderImages](../slices/SliderImages/index.vue) | Pinned one-slide-at-a-time slider with numbered bar navigation; scroll steps through the slides (70vh each, 50vh hold at both ends) and releases after the last, clicking a bar scrolls to that slide. The active slide's clip autoplays muted (clips fetched once the section is near, active + next slide only) and, when it ends, the slider scrolls on to the next slide (only while pinned in view; the last clip loops). The active bar fills with the clip and doubles as its scrubber: press/tap to seek, drag sideways to scrub (clip held until release), ←/→ step 10%; the image is the poster and fallback | `section_label`, `title` (optional), `slides` (group: `image`, `video_url` + `video_url_mobile` (optional), `title`, `description` (rich text: paragraphs, bullets, bold / italic / links)) |
| `stats_solution` | [StatsSolution](../slices/StatsSolution/index.vue) | Stat cards revealed by the scroll (scrubbed, not pinned): the beige top bar fades in, the blue cards fade in left to right (staggered across the grid, so stacked cards on mobile reveal one by one as they enter), then the bottom bar fades in as a blue track and fills yellow left to right while its figure counts up from 0 in step with the fill (the text flips to dark exactly at the fill edge). Shown fully revealed in the Slice Simulator and with reduced motion | `section_label`, `title`, `top` (single-item group: `label`, `value`, `caption`, `description`), `cards` (group, same fields), `bottom` (single-item group, same fields) |
