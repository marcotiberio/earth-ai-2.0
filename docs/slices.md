...
| `slider_images` | [SliderImages](../slices/SliderImages/index.vue) | Pinned one-slide-at-a-time slider with numbered bar navigation; scroll steps through the slides (70vh each, 50vh hold at both ends) and releases after the last, clicking a bar scrolls to the start of that slide. A slide's clip is scrubbed by the scroll across the span its slide is on screen (as HorizontalScroll), ending as the next slide takes over, and the active bar fills with it (clips fetched once the section is near, active + next slide only); the image is the poster and fallback | `section_label`, `title` (optional), `slides` (group: `image`, `video_url` + `video_url_mobile` (optional), `title`, `description` (rich text: paragraphs, bullets, bold / italic / links)) |
